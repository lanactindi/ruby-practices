...
class LS
  ...
  def exec
    ...
    return print_detail_lines(files) if @option[:l]

    print_files_names(files)
  end

  def print_files_names(files)
    columns = 3
  ...
  def print_detail_lines(files)
    files.map { |file| print_file_detail(file) }
  end
